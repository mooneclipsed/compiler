...
struct ASTnode *multiplicative_expr(void){
    ...
    // If no tokens, return just the left node
    ...
    while((tokentype == T_STAR) || (tokentype == T_SLASH)){
        ...
        // Updata the details of the current token.
    ...
    // Return whatever tree we have craeated
    return (left);
    
}

// Return an AST tree whose is a '+' or '-' binary operator
struct ASTnode* additive_expr(void){
    struct ASTnode *left, *right;
    int tokentype;
    ...
    // Retrun whatever tree we have created
    return (left);
}
